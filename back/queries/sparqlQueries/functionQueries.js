module.exports = {
  ...
  symbolsInfoQuery: function(value) {
    return (
      `
  ...
  AllInfoQuery: function(value) {
    return (
  `
  select  DISTINCT STR(?siblings) as ?Siblings, ?uri as ?resource, STR(?n) as ?nameOfGod, ?image, STR(?go) as ?GodOf,STR(?abode) as ?Abode, if(bound(?ge), STR(?ge), if(regex(?go,"God .*") , "Male","Female")) as ?Gender, STR(?symbol) as ?Symbol
  where {
    #Get only the Greek gods
    ?uri dbp:name ?n;
    dbp:godOf ?go;
